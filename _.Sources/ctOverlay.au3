...
Func _ProcMacro($sString,$isClip=0)
    ;If StringRegExp($sString,"\{@ClipSplit(?:\:([^}]))?\}") Then
    ;    Local $aMatch=StringRegExp($sString,"\{@ClipSplit(?:\:(\d{1,}))?\}",4)
    ;    Local $iLen,$m
        ...
        For $i=1 To $aCalc[0]
            ...
            Switch $sFunc
              ...
              Case "Replace"
                  ...
                  ;Local $aMyParm=StringSplit($aOp[1],'|')
                  ;ConsoleWrite($aOp[1]&@CRLF)
                  ;_ArrayDisplay($aParam)
                  If $aParam[0]>2 Then; Look for escaped commas.
...
Func _ctxClipTikClip()
    ...
    ;  If @error Then Return
    ;  For $i=0 To UBound($aMatch,1)-1
    ;    $sUrI="https://na.myconnectwise.net/v4_6_release/services/system_io/Service/fv_sr100_request.rails?service_recid="&$aMatch[$i]&"&companyName=corsica"
    ;    __ClipPutHyperlink($sUrI,'#'&$aMatch[$i])
    ;  Next
    ;EndIf
    If StringRegExp($sClip,"\b#?(\d{7})\b") Then
