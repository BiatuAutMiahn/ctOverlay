...
;             TheXman - Changed Json_Get() and Json_Put() to handle all valid key names
; ============================================================================================================================
...
Func Json_ObjExists(ByRef $Object, $Key)
	...
	For $x = 1 To $Keys[0]
		If $DynObject.Exists($Keys[$x]) Then
			...
			Else
				$DynObject = Json_ObjGet($DynObject, $Keys[$x])
				If Not IsObj($DynObject) Then Return False  ;Added by XTC
			EndIf
		Else
			Return False
		EndIf
...
Func Json_Put(ByRef $Var, $Notation, $Data, $CheckExists = False)
	...
	If IsArray($Match) Then
		...
		If IsString($Index) Then
			If $CheckExists And (Not Json_IsObject($Var) Or Not $Var.Exists($Index)) Then
...
Func Json_Get(ByRef $Var, $Notation)
	...
	If IsArray($Match) Then
		...
		Local $Item
		If IsString($Index) And Json_IsObject($Var) And $Var.Exists($Index) Then
			$Item = $Var.Item($Index)
