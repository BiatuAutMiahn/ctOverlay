...
If Not IsDeclared('wbemFlagReturnImmediately') Then Global Const $wbemFlagReturnImmediately = 0x10
If Not IsDeclared('wbemFlagForwardOnly') Then Global Const $wbemFlagForwardOnly = 0x20
...
Func _DisableNetAdapter($sNetAdapter)
	...
	Local $objNetAdapter , $iReturn = 0
	Local $sAdapterName = _GetNetworkAdapterFromID($sNetAdapter)
	If $sAdapterName Then $sNetAdapter = $sAdapterName
	Local $sQueryNetAdapterConfig = "select * from Win32_NetworkAdapter Where Name = '" & $sNetAdapter & "'"
	Local $colNetAdapterConfig = $objWMIService.ExecQuery($sQueryNetAdapterConfig, "WQL", $wbemFlagReturnImmediately + $wbemFlagForwardOnly)
...
Func _EnableDHCP($sNetAdapter)
	...
	Local $sAdapterName = _GetNetworkAdapterFromID($sNetAdapter)
	If $sAdapterName Then $sNetAdapter = $sAdapterName
	Local $sQueryNetAdapterConfig = "select * from Win32_NetworkAdapterConfiguration Where Caption like '%" & $sNetAdapter & "'"
	Local $colNetAdapterConfig = $objWMIService.ExecQuery($sQueryNetAdapterConfig, "WQL", $wbemFlagReturnImmediately + $wbemFlagForwardOnly)
	If NOT IsObj($colNetAdapterConfig) Then Return 0
	For $objNetAdapter In $colNetAdapterConfig
		...
		EndIf
	Next
	If $iReturn == Null Then $iReturn = 65
...
Func _EnableDHCP_DNS($sNetAdapter)
	...
	Local $sAdapterName = _GetNetworkAdapterFromID($sNetAdapter)
	If $sAdapterName Then $sNetAdapter = $sAdapterName
	Local $guid = _GetNetworkGUI($sNetAdapter)
...
Func _EnableStatic($sNetAdapter, $sIPAddress, $sSubnetMask)
	Local $objWMIService = ObjGet("winmgmts:\\" & @ComputerName & "\root\CIMV2")
	If $objWMIService = 0 Then Return 0
	Local $objNetAdapterConfig, $iReturn = 0
...
; ===============================================================================================================================
Func _FlushDNS()
	Local $aReturn = DllCall("dnsapi.dll", "BOOL", "DnsFlushResolverCache")
...
Func _FlushDNSEntry($sHost)
	Local $aReturn = DllCall("dnsapi.dll", "int", "DnsFlushResolverCacheEntry_W", "WSTR", $sHost)
	If @error Then Return SetError(@error, 0, 0)
 	Return $aReturn[0]
EndFunc ; ===> _FlushDNSEntry
...
Func _GetNetworkAdapterFromID($sNetworkID)
	...
	Next
	Return 0
EndFunc ; ===> _GetNetworkAdapterFromID
...
Func _GetNetworkAdapterInfos($sNetAdapter = "")
	...
	If $sAdapterName Then $sNetAdapter = $sAdapterName
	If $sNetAdapter <> "" Then
		$filter = "Description = '" & $sNetAdapter & "'"
	Else
	...
	Local $objWMIService = ObjGet("winmgmts:\\" & @ComputerName & "\root\CIMV2")
	If $objWMIService = 0 Then Return 0
	Local $sQueryNetAdapter = 'select * from Win32_NetworkAdapter WHERE ' & $filter
	ConsoleWrite($sQueryNetAdapter & @CRLF)
...
Func _GetNetworkAdapterList()
	Local $aInfos[1][2], $i = 1, $j = 1, $k = 0, $aGuids[1], $sGuid, $sIndex
	While 1
	...
	Return $aInfos
EndFunc ; ===> _GetNetworkAdapterList
...
Func _GetNetworkIDFromAdapter($sNetAdapter)
	...
	Next
	Return 0
EndFunc ; ===> _GetNetworkIDFromAdapter
...
Func _IsWirelessAdapter($sAdapter)
	Local $hDLL = DllOpen("wlanapi.dll"), $tInterface, $pInterface
	Local $aResult = DllCall($hDLL, "dword", "WlanOpenHandle", "dword", 2, "ptr", 0, "dword*", 0, "hwnd*", 0)
	If @error Or $aResult[0] Then Return 0
	Local $hClientHandle = $aResult[4]
	$aResult = DllCall($hDLL, "dword", "WlanEnumInterfaces", "hwnd", $hClientHandle, "ptr", 0, "ptr*", 0)
	If @error Or $aResult[0] Then Return 0
	Local $pInterfaceList = $aResult[3]
	Local $tInterfaceList = DllStructCreate("dword", $pInterfaceList)
	Local $iInterfaceCount = DllStructGetData($tInterfaceList, 1)
...
Func _ReleaseDHCPLease($sNetAdapter = "")
	...
	If $sAdapterName Then $sNetAdapter = $sAdapterName
	If $sNetAdapter = "" Then
		Local $objInstance = $objWMIService.Get("Win32_NetworkAdapterConfiguration")
...
; Syntax.........: _SetCategory($sNetworkId, $iNetCategory)
...
; ===============================================================================================================================
Func _SetCategory($sNetworkId, $iNetCategory)
...
Func _SetDNSServerSearchOrder($sNetAdapter, $aDNSServerSearchOrder)
	...
	If $iReturn == Null Then $iReturn = 65
	Return SetError($iReturn, "", 0)
EndFunc ; ===> _SetDNSServerSearchOrder
...
; Parameters.....: $aDNSDomainSuffixSearchOrder     - (ARRAY) List of server suffixes to query for DNS servers.
; Return values..: Success  - 1 or 2 (reboot required)
;                  Failure  - 0 and sets @error (see remarks)
;
; Remarks........: When the function fails (returns 0) @error contains extended information (see global remarks).
; ===============================================================================================================================
Func _SetDNSSuffixSearchOrder($aDNSDomainSuffixSearchOrder)
	Local $objWMIService = ObjGet("winmgmts:\\" & @ComputerName & "\root\CIMV2")
	Local $objNetworkSettings = $objWMIService.Get("Win32_NetworkAdapterConfiguration")
	If Not IsObj($objNetworkSettings) Then Return SetError(1, 0, 0)
	If NOT IsArray($aDNSDomainSuffixSearchOrder) Then Return 0
	Local $iReturn = $objNetworkSettings.SetDNSSuffixSearchOrder($aDNSDomainSuffixSearchOrder)
	If $iReturn == 0 Then Return 1
	If $iReturn == 1 Then return 2
	If $iReturn == Null Then $iReturn = 65
	Return SetError($iReturn, "", 0)
...
Func _SetGateways($sNetAdapter, $DefaultIPGateway, $GatewayCostMetric = 1)
	Local $objWMIService = ObjGet("winmgmts:\\" & @ComputerName & "\root\CIMV2")
	If $objWMIService = 0 Then Return 0
	Local $objNetAdapterConfig, $iReturn = 0, $aGWAddress[1], $aGWMetric[1]
	Local $sAdapterName = _GetNetworkAdapterFromID($sNetAdapter)
	If $sAdapterName Then $sNetAdapter = $sAdapterName
...
Func _WMIDate($sDate, $iFormat = 0)
	...
	Switch $iFormat
		Case 0
			$sDateFormat = "$1-$2-$3 $4:$5:$6"
		Case 1
			$sDateFormat = "$3/$2/$1 $4:$5:$6"
		Case 2
			$sDateFormat = "$1$2$3$4$5$6"
		Case Else
			Return ""
	...
	Local $newDate = StringRegExpReplace($sDate, "\A(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2}).*", $sDateFormat)
	Return $newDate
EndFunc ; ===> _WMIDate
