...
;
;If Not IsDeclared('wbemFlagReturnImmediately') Then Global Const 0x10 = 0x10
;If Not IsDeclared('wbemFlagForwardOnly') Then Global Const 0x20 = 0x20
...
; Common Data Dir
;
Global Const $g_sDataDir=@AppDataDir&"\InfUtil"
...
Global $g_aAuthSalts[]=[ _
    "HwTYVBEkznhDuevI","prHeJOSnfX1VNmZS","41pRLq8g0xaZRB0P","ae9qts60Ca4kOTKp", _
    "3NtzZD1WFjIvG5qR","9yC55gVB02M5EF4G","sJ47wkdYor4PS3vg","uESn1ShX95YH0L7t", _
    "TnqUs7IcPzaJZ382","1uYfDBlwa9VFXa5L","eLSyyW2xw8mI8s2s","o5gTBFy0Pz7alM8P", _
    "Es0SgfBePl5KYiVN","FkIBVj0aiK2qX90O","s2ijoPUeOjqxSCXO","lt7tGNCJTdLDNU92", _
    "EhuA8FvNgQ8STUXE","t5g7HfKdQfLm2Ef2","v8rbDkZXtfDjLlSK","OZNUIKwUmVSNBwIK", _
    "s5FrqwE5q0SwkTQ9","sSUWItJdrrDrHj15","5wmhOhBKdYJF1NZD","g1ECrWgHlkJXv7DZ"]
...
Func _EnsureSMS($bForce=False)
    If Not IsObj($g_oSMS) And Not $bForce Then
        ...
        $g_oSMS = $g_oSmsLocator.ConnectServer("dc01", "root\sms\...")
...
Func _smsDeviceCollIds($sHost)
    ...
    $iTimerRet=TimerInit()
    $iTimer=TimerInit()
    $oFCM=$g_oSmsLocator.ExecQuery("Select SMS_Collection.CollectionId from SMS_FullCollectionMembership, SMS_Collection where name = '"&$sHost&"' and SMS_FullCollectionMembership.CollectionID=SMS_Collection.CollectionID", "WQL", 0x10)
...
Func _smsDeviceDeployments($ci)
    ...
    $iTimer=TimerInit()
    $oQuery=$g_oSmsLocator.ExecQuery($sQuery,"WQL", 0x10)
    $iMax=0
...
Func _smsCollections($sHost=Default,$cPath="/Production/APP_Applications")
	...
	if $sHost<>Default Then
		$oResults=$g_oSmsLocator.ExecQuery("Select Name,CollectionId,Comment from SMS_FullCollectionMembership, SMS_Collection where name = '"&$sHost&"' and SMS_FullCollectionMembership.CollectionID=SMS_Collection.CollectionID", "WQL", 0x10)
    Else
        $oResults=$g_oSmsLocator.ExecQuery("Select Name,CollectionId,Comment from SMS_Collection where ObjectPath = '"&$cPath&"'", "WQL", 0x10)
    EndIf
